...
function selectDevice(deviceId) {
  ...
  document.getElementById('detailTitle').innerHTML = `<span id="detailName">${deviceId.substring(0, 12)}...</span> <button class="btn btn-glass btn-sm" onclick="editDeviceName()" style="font-size:11px;vertical-align:middle">&#9998; Rename</button> <button class="btn btn-glass btn-sm" onclick="deleteDevice('${deviceId}')" style="font-size:11px;vertical-align:middle;color:#ff5252">🗑 Delete</button>`;
...
async function deleteDevice(id) { const deviceId = id || selectedDevice; if (!deviceId) return; showConfirm('Delete Device', 'Permanently delete this device and all its data.', async () => { try { await fetch(`${API_BASE}/api/device/${deviceId}`, { method: 'DELETE' }); showToast('Deleted'); showDeviceList(); refreshDevices(); refreshStats(); } catch (_) { showToast('Failed', true); } }); }
